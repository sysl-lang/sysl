    ...
    // Armv7E-M, on the MPS2-AN500's Cortex-M7. `start_thumb.s` is the nearest sibling and most of
    // this is the same; the two places it differs are the vector table's length and the FPU, and
    // both follow from the same fact — **this core has no TrustZone**.
    //
    // So there is no SecureFault, and slot 7 is reserved rather than a handler. `thumbv7em.ld` says
...
_start:
    ...
    // **Enable the FPU before anything can use it.** The triple is `thumbv7em-none-eabihf`, so the
    // ABI passes a `double` in `d0`/`d1` and the back end is free to reach for VFP anywhere -- and a
