 ...
 *
 * Two subjects are beside this rather than in it: a `val` whose payload is a raw pointer is
 * `ValPointerTests`, and a `val` inside a body — with the refusals and the name collisions — is
 * `ValLocalTests`.
 */
