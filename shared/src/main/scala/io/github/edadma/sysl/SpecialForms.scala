...
trait SpecialForms extends Closures {
        ...
        case None =>
          if !conforms("Display", ty) then
            ...
            val fix = unmetBound("Display", ty).getOrElse(ty match
              ...
              // prelude's and nothing in `[]int` is this module's, so the block named here is one
              ...
              case _: Type.Array | _: Type.Slice if implementableHere(ty) =>
                s"write an 'impl Display for ${show(ty)}' to say how it renders"

              case _: Type.Array | _: Type.Slice =>
                s"nothing renders a ${show(ty)}, and an 'impl Display' for it has no home outside " +
                  "the prelude — print the elements, or give them a type of your own to be held in"
   ...
   *
   * `Display` belongs to the prelude, so the only thing that can give the block a home is a type of
