...
class EnumAttrRunTests extends AnyFreeSpec with RunSupport {
  ...
  // `reference/types.md § Enums` says `::` is what keeps these out of the member namespace, and a
  // variant named after an attribute is the case that would decide it if they shared one.
  // `Last::First` reads across both spellings at once: the attribute on the left of `::`, the
  // variant on the right of `.`.
