...
trait Hoisting extends HoistMembers {
  ...
  protected def hoistType(stmt: Stmt): Unit = stmt match
    case s: StructDecl =>
      ...
      if Library.owns(s) then libraryNames(s.name) = key
    case e: EnumDecl =>
      ...
      if Library.owns(e) then libraryNames(e.name) = key
      ...
      for v <- e.variants do
        ...
        if Library.owns(e) then libraryNames(v.name) = vkey
    case t: TraitDecl =>
      ...
      if Library.owns(t) then libraryNames(t.name) = key
  ...
  protected def hoistFunc(stmt: Stmt): Unit = stmt match
    case original: FuncDecl =>
      ...
      if Library.owns(f) then libraryNames(f.name) = key
