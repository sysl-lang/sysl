...
object LibraryArtifact {
  ...
  /** Where a prebuilt standard module lives when nobody says otherwise: what `build-lib --core`
   * writes with no `-o`, and what a compilation looks for having been given no `--core-lib`.
   *
   * One path for both ends is the point — it is what makes the artifact something built once after a
   * clone and then never thought about again, rather than a file whose location has to be carried
   * around on every command line. Relative, so it belongs to the tree it was built from: a worktree
   * has its own `lib/sysl` and therefore its own artifact, and the fingerprint would reject another
   * one anyway.
   *
   * Not committed, and deliberately: the object half is built for one machine while sysl
   * cross-compiles, and 0.7s of build is a poor trade for a binary in the tree that must be
   * regenerated by hand on every edit to the library.
   */
  val coreDefault: String = s".sysl/core$extension"
